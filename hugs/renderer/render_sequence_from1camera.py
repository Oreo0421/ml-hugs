#!/usr/bin/env python3
"""
Batch render script to process frames of human PT files with scene

FEATURES:
- Automatically creates output directory if it doesn't exist
- Validates all input paths before processing
- Uses only the FIRST camera from camera_params.json for all frames
- Supports rendering: human only, scene only, or human+scene combined

USAGE EXAMPLE:
python render_sequence_firstcamera.py \
  --human_pt_dir "transformed_humans/pt" \
  --scene_pt "output/gs_out/scene_gs_out.pt" \
  --output_dir "/mnt/data_hdd/fzhi/output/101010/djr/top" \
  --start_frame 0 \
  --end_frame 99 \
...
def save_rendered_image(image_tensor, save_path, frame_idx=None):
    ...
    if frame_idx is not None:
        logger.info(f"Frame {frame_idx:08d} rendered and saved to {save_path}")


def load_camera_data(camera_json_path, frame_idx=0):
    """Load the FIRST camera data from JSON file for all frames"""
    with open(camera_json_path, 'r') as f:
        cam_j = json.load(f)

    i = 0  # always use the first camera
...
def main():
    ...
    parser.add_argument("--human_pt_dir", type=str, required=True,
                        help="Directory containing human PT files")
    parser.add_argument("--scene_pt", type=str, required=True,
                        help="Path to scene PT file")
    ...
    parser.add_argument("--render_mode", type=str, default='human_scene',
                        ...
                        help="Rendering mode: human, scene, or human_scene")
    parser.add_argument("--bg_color", type=float, nargs=3, default=[0.0, 0.0, 0.0],
                        help="Background color RGB (default: black)")
    ...
    parser.add_argument("--scaling_modifier", type=float, default=1.0,
                        help="Scaling modifier for rendering")
    ...
    if not human_pt_dir.exists():
        logger.error(f"✗ Human PT directory does not exist: {human_pt_dir}")
        raise FileNotFoundError(f"Human PT directory not found: {human_pt_dir}")
    else:
        logger.info(f"✓ Human PT directory found: {human_pt_dir}")
    
    if not os.path.exists(args.scene_pt):
        logger.error(f"✗ Scene PT file does not exist: {args.scene_pt}")
        raise FileNotFoundError(f"Scene PT file not found: {args.scene_pt}")
    else:
        logger.info(f"✓ Scene PT file found: {args.scene_pt}")
    
    if not os.path.exists(args.camera_json):
        logger.error(f"✗ Camera JSON file does not exist: {args.camera_json}")
        raise FileNotFoundError(f"Camera JSON file not found: {args.camera_json}")
    else:
        logger.info(f"✓ Camera JSON file found: {args.camera_json}")
    ...
    try:
        ...
        logger.error(f"✗ Permission denied when creating directory {output_dir}: {str(e)}")
        raise
    ...
    logger.info(f"Successfully rendered: {successful_frames}/{total_frames} frames")
    if failed_frames:
